  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const memoDetailsResponse = await axios.get(`/api/memo/created_details/${memoId}?email=${userEmail}`);
        const details = memoDetailsResponse.data.memo;
        setMemoDetails(details);

        // Fetch PDF URL
        const pdfResponse = await axios.get(`/api/memo/created/${memoId}?email=${userEmail}`, {
